...
<script type="text/javascript">
	$(function() {
		...
		$("#userNumber").blur(function() {
			//ajax异步验证账号
			$.post("system/userCheck", {
				userNumber : $(this).val()
			}, function(data) {

				if (data == "true") {
					$("#errorinfo").html("账号已经存在，请从新输入！");
				} else {
					$("#errorinfo").html("");
				}
...
<body>
	...
	<form action="UserAddServlet" class="form-horizontal" method="post"
		...
		<div class="row">
			...
			<div class="col-xs-5">
				<div class="form-group ">
					...
					<div class="col-xs-5 ">
						<input type="text" name="userBirthday" value="2000-01-01 00:00:00"
				...
				<div class="form-group ">
					...
					<div class="col-xs-9">
						<input type="text" name="userNumber" id="userNumber"
							class="form-control" data-rule="用户账号:required" placeholder="请输入用户账号" />
					</div>
				</div>
			</div>
			<div class="col-xs-5">
				<div class="form-group ">
					...
					<div class="col-xs-9 ">
						<input type="text" name="userPw" data-rule="用户密码:required" class="form-control"
							placeholder="请输入用户密码" />
					</div>
				</div>
			</div>
